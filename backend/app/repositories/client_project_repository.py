...
class ClientProjectRepository:
    ...
    def create(
        ...
        query = text("""
            ...
            RETURNING id, client_name, project_title, description, technologies, outcome, testimonial, project_url,
                      ...
                      project_image_type, is_featured, completed_at, created_at, updated_at;
        """)
    ...
    def get_image(db: Session, project_id: int) -> dict[str, Any] | None:
        """Fetch only the image bytes for one client project. See ProjectRepository.get_image."""
        query = text("""
    ...
    @staticmethod
    def update(db: Session, project_id: int, **kwargs) -> dict[str, Any] | None:
