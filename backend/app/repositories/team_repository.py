...
class TeamRepository:
    ...
    def create(
        ...
        query = text("""
            ...
            RETURNING id, name, role, bio, skills, linkedin_url, portfolio_url,
                      (profile_image_data IS NOT NULL) AS has_image,
                      profile_image_type, experience_level, created_at, updated_at;
        """)
    ...
    @staticmethod
    def get_by_id(db: Session, member_id: int) -> dict[str, Any] | None:
    ...
    @staticmethod
    def get_image(db: Session, member_id: int) -> dict[str, Any] | None:
        """Fetch only the image bytes for one team member. See ProjectRepository.get_image."""
        query = text("""
            SELECT profile_image_data, profile_image_type, updated_at
            FROM team_members
            WHERE id = :member_id AND profile_image_data IS NOT NULL;
        """)
        row = db.execute(query, {"member_id": member_id}).fetchone()
        return dict(row._mapping) if row else None

    @staticmethod
    def update(db: Session, member_id: int, **kwargs) -> dict[str, Any] | None:
    ...
    def delete(db: Session, member_id: int) -> bool:
        ...
        result = db.execute(
            text("DELETE FROM team_members WHERE id = :member_id;"), {"member_id": member_id}
        )
        db.commit()
