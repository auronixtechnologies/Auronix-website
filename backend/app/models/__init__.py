...
from sqlalchemy import Column, Integer, String, Text, JSON, DateTime, Boolean, ForeignKey, Table, LargeBinary
...
class TeamMember(Base):
    ...
    profile_image_type = Column(String(50), nullable=True)  # MIME type (e.g., 'image/jpeg', 'image/png')
    ...
    projects_members = relationship("Project", secondary=project_members, back_populates="team_members")
...
class Project(Base):
    ...
    project_image_type = Column(String(50), nullable=True)  # MIME type (e.g., 'image/jpeg', 'image/png')
    ...
    # Relationships
    creator = relationship("TeamMember", back_populates="projects_created", foreign_keys=[created_by])
    team_members = relationship("TeamMember", secondary=project_members, back_populates="projects_members")

    def __repr__(self):
        return f"<Project(id={self.id}, title={self.title}, domain={self.domain})>"
